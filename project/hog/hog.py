...
def more_boar(player_score, opponent_score):
    ...
    if player_score >= opponent_score:
        return False
    a = str(player_score)   if player_score >= 10 else '0' + str(player_score)
    b = str(opponent_score) if opponent_score >= 10 else '0' + str(opponent_score)
...
def make_averaged(original_function, trials_count=1000):
    ...
    >>> dice = (4, 2, 5, 1)
    >>> averaged_dice = make_averagedmake_test_dice(roll_dice, 1000)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if score+piggy_points(opponent_score) >=goal:
        return 0 
