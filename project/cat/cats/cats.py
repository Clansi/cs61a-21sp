...
def about(topic):
    ...
    def f(para):
        for i in topic:
            if i in split(lower(remove_punctuation(para))):
               return True
        return False
    return f
    # END PROBLEM 2
...
def sphinx_switches(start, goal, limit):
    ...
    def helper(a,b,count):
        ...
        if len(a) == 0 or len(b) == 0:
            return count+x
        if a[0] == b[0]:
            return helper(a[1:],b[1:],count)
        else:
            return helper(a[1:],b[1:],count+1)
...
def pawssible_patches(start, goal, limit):
    ...
    >>> big_limit = 10
    >>> pawssible_patches("cats", "scat", big_limit)       # cats -> scats -> scat cats kats
    2
    >>> pawssible_patches("purng", "purring", big_limit)   # purng -> purrng -> purring ng ring
    2
...
def time_per_word(times_per_player, words):
    ...
    for i in times_per_player:
        res.append([i[a] -i[a-1] for a in range(1,len(i))])
