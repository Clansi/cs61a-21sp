...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        op1 = scheme_eval(first,env)
        validate_procedure(op1)
        op2 = rest.map(lambda x : scheme_eval(x,env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        res = do_lambda_form(Pair(target.rest,expressions.rest),env)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    res =LambdaProcedure(formals,expressions.rest,env)
    return res
    # END PROBLEM 8
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        eval1 = scheme_eval(expressions.first,env)
        if is_true_primitive(eval1):
            return eval1
        expressions = expressions.rest
    return False
    # END PROBLEM 12
